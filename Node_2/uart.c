...
RingBuf ringBuffer = {0}; // Renamed global variable to avoid conflict
...
void uart_init(uint32_t cpufreq, uint32_t baudrate) {
	...
	UART->UART_BRGR = 547; // Using 84 MHz clock, 9600 baud rate
...
void UART_Handler() {
	...
	if (status & UART_SR_RXRDY) {
		if (!push(&ringBuffer, UART->UART_RHR & 0xff)) {
